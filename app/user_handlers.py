...
async def allowed_messages_handler(message: Message) -> None:
    if result := check_message(message):
        ...
        db_create.add_new_message_to_db(message, channel_message.message_id)
        await message.answer("Ваше сообщение успешно отправлено")


def check_message(message: Message) -> str | None:
    if not db_read.check_for_user_existence(message.from_user.id):
        db_create.add_new_user_to_db(message)
    elif message.date.timestamp() < db_read.get_prohibit_sending_time():
        return f"Администратор заблокировал отправку сообщений до {datetime.fromtimestamp(db_read.get_prohibit_sending_time()).strftime("%H:%M:%S %d.%m.%Y")}"
    elif db_read.check_user_for_ban(message.from_user.id):
        return "Вы были заблокированы администратором. Если Вы считаете, что блокировка несправедлива, то пишите @username"
    elif not db_read.check_time_for_sending(message):
